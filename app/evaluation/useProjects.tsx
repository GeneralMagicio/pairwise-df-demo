...
const getProjectRationales = async (rationaleQuery: IRationaleQuery): Promise<IReturnRationaleQuery> => {
    const response = await axiosInstance.get('/project/rationales',{data: rationaleQuery});
...
export const useGetProject = (id: number) => {
  ...
  export const useGetProjectPairCategory = async (id1: number, id2: number) => {
    return useQuery({
        queryKey: ['project-pair',id1,id2],
        queryFn: () => getProjectPairCategory(id1,id2),
    })
  }
export const useGetProjectRationales = (page: number,limit: number,createdAtGte: string, createdAtLte: string, projectIds: number[],myEvaluation: boolean)=> {
    return useQuery({
